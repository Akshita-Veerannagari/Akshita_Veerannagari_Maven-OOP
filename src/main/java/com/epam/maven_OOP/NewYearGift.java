...
public class NewYearGift 
{
	...
	NewYearGift(int sw_no, int ch_no ,String name)
	{
		child_name=name;
		sweets = new sweets[sw_no];
		choc = new chocolates[ch_no];
	}
	...
	public static void main(String args[])
	{
		 ...
		 for(int h=0;h<t;h++)
		 {
			...
			gift[h] = new NewYearGift(no_of_sweets_type,no_of_chocolates_type,child_name);
			...
			for(int i=0;i<no_of_sweets_type;i++)
			{
				...
				sw_count+=quantity;
				sweets[i]=new sweets(sweet_weight,sweet_price,quantity);
			...
			for(int i=0;i<no_of_chocolates_type;i++)
			{
				...
				choc_count+=quantity;
				choc[i]=new chocolates(choc_weight,choc_price,quantity,flavour);
			...
			if(no_of_chocolates_type!=0)
			{
				...
				int op=sc.nextInt();
				Arrays.sort(choc,new choc_sort(op));
